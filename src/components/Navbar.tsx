...
import { Globe, Menu, X, Shield, ArrowRight, Video, Lock } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <Link href={`/${locale}`} className="flex items-center gap-2 group">
            ...
            <span className="text-xl font-semibold tracking-tight text-zinc-900 dark:text-white group-hover:text-emerald-600 transition-colors">
              Vanyshe
            </span>
            <span className="hidden sm:inline-block text-[11px] uppercase tracking-wider font-mono px-2 py-0.5 rounded border border-zinc-200 dark:border-zinc-800 text-zinc-500">
              Ephemeral
            </span>
          </Link>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          {/* Language Selector */}
          <div className="flex items-center rounded-lg border border-zinc-200 dark:border-zinc-800 p-0.5 bg-zinc-100/60 dark:bg-zinc-900/60 text-xs">
            {languages.map((lang) => (
              <button
                key={lang.code}
          ...
          <Link
            href="/admin"
            className="hidden lg:inline-flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-900 dark:hover:text-white px-2.5 py-1 rounded border border-transparent hover:border-zinc-200 dark:hover:border-zinc-800 transition-colors"
          >
          ...
          <button
            ...
            disabled={creating}
            className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-zinc-900 hover:bg-zinc-800 dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200 rounded-lg shadow-sm transition-all disabled:opacity-50"
          >
